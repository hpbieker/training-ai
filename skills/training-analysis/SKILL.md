...
- The `--brief` output includes key efforts, peaks, the hardest block, and first-pass HR recovery context for chat analysis.
...
- Add target or threshold detection when identifying work blocks from power, for example `--target 300 --tolerance 12 --min-block 10m` or `--threshold 190 --min-block 3m`.
